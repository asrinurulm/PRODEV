...
class pkpController extends Controller
{
    ...
    public function approvesamplepkp($id_project){
        $pkp = pkp_project::where('id_project',$id_project)->first();
    ...
    public function updatetipp(Request $request,$id_project,$revisi,$turunan){
        ...
        $data->save();
            ...
            if($clf>0){
                ...
                $tip->remarks_sesforecash=$request->$remarks_sesforecash;
            ...
            if ($validator->passes()) {
				...
				for ($i = 0; $i < count($ids); $i++)
				{
                    ...
                    $pipeline->id_pkp=$isises->id_pkp;
                    $pipeline->revisi=$isises->revisi;;
			...
			if($request->klaim!=''){
				...
				if ($validator->passes()) {
					...
					for ($i = 0; $i < count($ids); $i++)
					{
						...
						$detail->id_pkp=$isises->id_pkp;
						$detail->revisi=$isises->revisi;;
            ...
            try{
                Mail::send('pv.aktifitasemail', ['type'=>'PKP',],function($message)use($request)
                {
                    $tujuan = array(); 
    ...
    public function upversionpkp($id_project,$revisi,$turunan){
        ...
        $data->save();
            ...
            if($clf>0){
                ...
                $tip->remarks_sesforecash=$pkpp->$remarks_sesforecash;
